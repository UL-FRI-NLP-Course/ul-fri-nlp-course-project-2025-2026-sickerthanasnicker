...
\section*{Introduction}
	This project focuses on building an AI chat assistant for the Slovenian law domain. The main goal is
	to help users get quick and understandable answers to legal questions while still keeping the answers
	grounded in official legal sources.
...
At this stage, our main idea is to base the system on retrieval-augmented generation. Instead of
letting the model answer only from its internal knowledge, we would first retrieve relevant passages
from official Slovenian legal documents and then generate the answer from those passages. This should
help improve accuracy and also make it easier to show the user where the answer comes from.

To reduce hallucinations, we would like the assistant to stay close to the retrieved documents,
include references to the source texts, and clearly say when the available information is not enough
for a reliable answer. We also expect that careful chunking of long documents, filtering by source
type, and reranking of retrieved passages could further improve the quality of the final responses.
